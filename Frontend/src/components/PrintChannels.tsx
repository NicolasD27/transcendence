...
const PrintChannels : React.FC<PropsPrintChannelsToJoin> = (props) => {
	...
	const sendPassword = (e: React.KeyboardEvent<HTMLInputElement> | any) => {
		axios
			.post(`http://${process.env.REACT_APP_HOST || "localhost"}:8000/api/channels/${props.channel.id}/join`, {"password" : enteredPassword}, { withCredentials: true })
		...
		<div className='channel'>
			{
                ...
                <>
                    ...
                    <button id="channelChat_button" onClick={() => props.setChatParamsState({'chatState' : !props.chatParamsState.chatState, 'id' : props.channel.id , 'chatName' : props.channel.name , 'chatPseudo' : props.channel.name, type : 'channel'})}/>
                </>
            }
            {
                !props.isMember &&
